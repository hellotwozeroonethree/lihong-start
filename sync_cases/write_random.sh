...
array_out_file="random_array_write_""$n_bs"
echo ${array_origin[*]} > $array_out_file
array_len=${#array_origin[@]}
echo "length of the arry_origin:"
echo $array_len

time_log="time_log_random_write_""$n_bs"
if [ -f "$time_log" ];then
    rm -f "$time_log"
fi

# do nvme read write command test
echo "Write begins at ...:" > "$time_log"
date >> "$time_log"
